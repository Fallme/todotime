...
### 给人看（Human Summary）
- **改了什么**：从主任务或子任务的播放按钮开始专注时，现在会稳定播放与主计时器“开始”按钮相同的双音提示。
- **为什么**：任务播放按钮会阻止点击事件继续冒泡，首次直接从任务开始时，原音频初始化监听可能收不到这次交互，导致开始提示音漏播。
- **影响范围**：主任务和子任务的快速开始提示音；设置中关闭提示音后仍保持静音，计时与任务归属逻辑不变。

### 给 AI 看（Technical Details）
- **涉及文件**：
  - `src/App.tsx` — 首次点击的 `initAudio()` 监听改为捕获阶段并使用 `once: true`，在 React 任务按钮处理器执行前准备音频，不受按钮 `stopPropagation()` 影响。
  - `test/timerInteractions.test.ts` — 新增主任务/子任务快速开始均调用 `timer.startWork()`、`startWork()` 播放 `playStart`、音频监听运行于捕获阶段的回归断言。
  - `README.md`、`docs/FEATURES.md`、`docs/TEST_REPORT.md` — 同步提示音规则和回归场景。
- **接口 / 数据模型变化**：无。
- **关键实现细节 / 注意事项**：捕获阶段只负责提前创建音频播放器；真正播放仍由 `useTimer.startWork()` 内的 `playSound(playStart)` 完成，因此继续遵守 `soundEnabled` 设置。工作计时已运行时点击其他任务仍只切换归属，不会重新播放或重置计时。
- **验证方式**：`npm test`（逻辑测试、ESLint、前端生产构建、API 类型检查）；`git diff --check`。
- **后续待办 / 已知问题**：无。
